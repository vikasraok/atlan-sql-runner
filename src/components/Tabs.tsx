        ...
        <div className=" flex items-center gap-x-2 overflow-x-auto [&::-webkit-scrollbar]:hidden hover:[&::-webkit-scrollbar]:block hover:[&::-webkit-scrollbar]:h-1.5 hover:[&::-webkit-scrollbar-thumb]:bg-slate-300 hover:[&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-track]:bg-transparent" role="tablist" data-testid="tabs">
            {tabs.map((tab) => (
                <div
                    ...
                    tabIndex={0}
                    className={`flex items-center gap-2 py-1 px-3 h-10  ${tab.id === activeId ? "bg-white border-blue-500 border border-b-transparent z-15" : "bg-white border-b border border-slate-200 hover:border-blue-400"} rounded-t-md cursor-pointer select-none `}
                ...
                >
                    ...
                    {tabs.length > 1 && <button
                        ...
                        className="ml-2 text-slate-500 hover:text-slate-700 px-2 py-1 rounded focus:ring-blue-500"
                        onClick={(e) => {
                            e.stopPropagation();
                            closeTab(tab.id);
                        }}
