...
describe('App Component', () => {
  ...
  it('renders the main header with database icon and title', async () => {
    ...
    const title = await screen.findByText('Atlan SQL Runner')
  ...
  it('has proper layout structure with correct CSS classes', async () => {
    ...
    await waitFor(() => {
      const mainContainer = screen.getByText('Atlan SQL Runner').closest('.h-screen')
      expect(mainContainer).toHaveClass('h-screen', 'flex', 'flex-col', 'bg-slate-50', 'text-slate-700')
      
      // Check header
      const header = screen.getByText('Atlan SQL Runner').closest('header')
